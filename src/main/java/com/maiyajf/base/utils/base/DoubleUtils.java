...
public class DoubleUtils {
    ...
    /**
     * 6. * 提供精确的加法运算 7. * @param v1 被加数 8. * @param v2 加数 9. * @return 两个参数的和 10.
     */
    ...
    /**
     * 18. * 提供精确的减法运算 19. * @param v1 被减数 20. * @param v2 减数 21. * @return 两个参数的差 22.
     */
    ...
    /**
     * 30. * 提供精确的乘法运算 31. * @param v1 被乘数 32. * @param v2 乘数 33. * @return 两个参数的积 34.
     */
    ...
    /**
     * 42. * 提供（相对）精确的除法运算,当发生除不尽的情况时, 43. * 精确到小数点以后10位,以后的数字四舍五入. 44. * @param v1 被除数 45. * @param v2 除数 46. * @return
     * 两个参数的商 47.
     */
    ...
    /**
     * 53. * 提供（相对）精确的除法运算. 54. * 当发生除不尽的情况时,由scale参数指 定精度,以后的数字四舍五入. 55. * 56. * @param v1 被除数 57. * @param v2 除数 58.
     * * @param scale 表示需要精确到小数点以后几位 59. * @return 两个参数的商 60.
     */
    ...
    /**
     * 72. * 提供精确的小数位四舍五入处理 73. * @param v 需要四舍五入的数字 74. * @param scale 小数点后保留几位 75. * @return 四舍五入后的结果 76.
     */
     ...
     * @since [产品/模块版本](可选)
     */
    public static String toYuanMoney(double amount, double special) {
        DecimalFormat df = new DecimalFormat("#0.00");
        if (special == amount)
            return null;
        if (compareMoney(amount, 0) == 0)
            return df.format(amount);
        return df.format(roundMoney(divide(amount, 100)));
    }
